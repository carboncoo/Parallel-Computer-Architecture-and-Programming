...
./scripts/nodemanager_local.py $debug_pyflags 1490 --log_dir=logs $debug_cflags $configflags &

nodemanager_pid=$!
sleep .5

# Start the master and sleep a moment to make sure it is listening.
./master --max_workers $1 --address=$(hostname):15418 --log_dir=logs $debug_cflags $(hostname):1490 &
master_pid=$!
sleep .5
