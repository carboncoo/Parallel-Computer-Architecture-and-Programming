...
void *worker_wrapper(void* request)
{
  
  Request_msg req = *((Request_msg *)request);
  ...
  } else if (req.get_arg("cmd").compare("computeprimes") == 0){

    // actually perform the work.  The response string is filled in by
    // 'execute_work'
    printf("are we here\n");
    Request_msg req2(req.get_tag());
    req2.set_arg("n", req.get_arg("n"));

    execute_work(req2, resp);
    std::string temp = std::string(req.get_arg("num")) + "~" + resp.get_response();
...
void worker_handle_request(const Request_msg& req) {
  ...
  Response_msg resp(req.get_tag());
  pthread_t worker;
  Request_msg *newmsg = new Request_msg(req);
  pthread_create(&worker, NULL, &worker_wrapper,newmsg);
}
