...
__global__ void scan_block(int* array, int bundleWidth){ //bundleWidth is number of circles
	...
	int globalWarpId = globalBundleBase + blockIdx.y*PREFIXBLOCKSIZE + warpid;
	...
	if(lane == (WARP_WIDTH-1)){
		array[globalWarpId] = array[globalBundleIndex]; //Step 2: copy partial-scan bases
	}
...
__global__ void thread_add(int* array, int scaned_width, int bundleWidth, int combineValGridY, int* val, int boundary){
	int index = blockIdx.y*PREFIXBLOCKSIZE + scaned_width + threadIdx.x;
	int i=1;
	while(index>=i*boundary)i++;
	i--;
	if(index>=boundary*i+scaned_width){
		int add = val[blockIdx.x*combineValGridY + index/scaned_width - 1];
		index += blockIdx.x*bundleWidth;		
		array[index] += add;
	}
}
...
if(blockIdx.x==1 && blockIdx.y==0 && threadIdx.x==0){
	...
	vals[combineValGridY*blockIdx.x + blockIdx.y*PREFIXBLOCKSIZE + threadIdx.x] = val;
}
...
/*
if(moveToIndex==5){
	printf("valid[5] is set to 1, blockx:%d, blocky:%d, threadx:%d\n", blockIdx.x, blockIdx.y, threadIdx.x);
}			
if(blockIdx.x==1 && blockIdx.y==0 && threadIdx.x==2){
	printf("globalIndex:%d, base:%d, moveToIndex:%d, scaned[globalIndex]:%d, valid[globalIndex]:%d, set zero:%d\n", globalIndex, base, moveToIndex, scaned[globalIndex], valid[globalIndex], (scaned[globalIndex]+base));
}
*/
		}
		if(bundleIndex==bundleWidth-1){
			if(scaned[globalIndex]<bundleWidth){
				valid[scaned[globalIndex]+base] = 0;
			}
//printf("Zero out:%d\n", (scaned[globalIndex]+base));
		}
	}
}
...
startTime = CycleTimer::currentSeconds();
	...
	int* valids;
	errchk = cudaMalloc((void **)&valids, sizeof(int) * bundlesTotal);
	if(errchk != cudaSuccess){
		printf("Render, cudaMalloc valids no enough memory!\n");
		exit(1);
	}
	int* hostValids = (int*)calloc(sizeof(int), bundlesTotal);
	cudaMemcpy(valids, hostValids, bundlesTotal * sizeof(int), cudaMemcpyHostToDevice);
	
	int* valids_copy;
	errchk = cudaMalloc((void **)&valids_copy, sizeof(int) * bundlesTotal);
	if(errchk != cudaSuccess){
		printf("Render, cudaMalloc valids_copy no enough memory!\n");
		exit(1);
	}
endTime = CycleTimer::currentSeconds();
printf("Render memory allocation:\t\t%.4f ms\n", 1000.f * (endTime - startTime));
	
startTime = CycleTimer::currentSeconds();
    kernelRenderCircles<<<gridDim, blockDim>>>(bundles, valids, bundlesWidth, numCircles);
    cudaThreadSynchronize();
endTime = CycleTimer::currentSeconds();
printf("Render kernelRenderCircles:\t\t%.4f ms\n", 1000.f * (endTime - startTime));

startTime = CycleTimer::currentSeconds();
	//prefix sum of valid and move bundles to put valid bundles to front of bundles array
...
//*
cudaMemcpy(hostValids, valids, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("Before scan_block, valids: ");
for(int i=numCircles;i<numCircles*2;i++){
	printf("%d,", hostValids[i]);
}
printf("\n");
//*/
...
//*
cudaMemcpy(hostValids, valids, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("After scan_block, valids: ");
for(int i=numCircles;i<numCircles*2;i++){
	printf("%d,", hostValids[i]);
}
printf("\n");
//*/
	...
	if(gridDimY > 1){
		int* device_val;
		errchk = cudaMalloc((void **)&device_val, sizeof(int) * gridDimY * gridDimX);
		if(errchk != cudaSuccess){
			printf("Render, prefix sum device_val no enough memory!\n");
			exit(1);
		}
...
/*
cudaMemcpy(hostValids, device_val, combineValGridY * gridDimX * sizeof(int), cudaMemcpyDeviceToHost);
printf("After scan_combine, device_val: ");
for(int i=combineValGridY;i<combineValGridY+PREFIXBLOCKSIZE;i++){
	printf("%d,", hostValids[i]);
}
printf("\n");
//*/
			...
			thread_add<<<threadAddGridDim, PREFIXBLOCKSIZE>>>(valids, scaned_width, numCircles, combineValGridY, device_val, scaned_width*PREFIXBLOCKSIZE);
			cudaThreadSynchronize();

/*
cudaMemcpy(hostValids, valids, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("After scan_combine, valids: ");
for(int i=0;i<numCircles;i++){
	printf("%d,", hostValids[i]);
}
printf("\n");
...
//*/
		...
		thread_add<<<threadAddOuterGridDim, PREFIXBLOCKSIZE>>>(valids, scaned_width, numCircles, combineValGridY, device_val, scaned_width*PREFIXBLOCKSIZE);
		cudaThreadSynchronize();
...
/*
RenderData* hostBundles = (RenderData*)calloc(sizeof(RenderData), bundlesTotal);
cudaMemcpy(hostBundles, bundles, bundlesTotal * sizeof(RenderData), cudaMemcpyDeviceToHost);
printf("Before move_bundle, bundles: ");
for(int i=0;i<bundlesTotal;i++){
	printf("(%d, %f, %f, %f), ", hostBundles[i].circleIndex, hostBundles[i].p.x, hostBundles[i].p.y, hostBundles[i].p.z);
}
printf("\n");
//*/
/*
cudaMemcpy(hostValids, valids_copy, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("Before move_bundle, valids_copy: ");
for(int i=0;i<numCircles;i++){
	printf("%d, ", hostValids[i]);
}
printf("\n");
...
/*
cudaMemcpy(hostValids, valids, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("Before move_bundle, valids: ");
for(int i=0;i<numCircles;i++){
	printf("%d, ", hostValids[i]);
}
printf("\n");
...
/*	
cudaMemcpy(hostBundles, bundles, bundlesTotal * sizeof(RenderData), cudaMemcpyDeviceToHost);
printf("After move_bundle, bundles: ");
for(int i=0;i<bundlesTotal;i++){
	printf("(%d, %f, %f, %f), ", hostBundles[i].circleIndex, hostBundles[i].p.x, hostBundles[i].p.y, hostBundles[i].p.z);
}
printf("\n");
...
/*
cudaMemcpy(hostValids, valids_copy, bundlesTotal * sizeof(int), cudaMemcpyDeviceToHost);
printf("After move_bundle, valids_copy: ");
for(int i=0;i<numCircles;i++){
	printf("%d, ", hostValids[i]);
}
printf("\n");
...
endTime = CycleTimer::currentSeconds();
printf("Render prefix sum:\t\t\t%.4f ms\n", 1000.f * (endTime - startTime));
...
startTime = CycleTimer::currentSeconds();
	...
	cudaThreadSynchronize();
endTime = CycleTimer::currentSeconds();
printf("Render renderPixels:\t\t\t%.4f ms\n", 1000.f * (endTime - startTime));
