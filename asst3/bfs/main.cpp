...
int main(int argc, char** argv) {
    ...
    //* execute top-down parallel version
    start_time = CycleTimer::currentSeconds();
    bfs_top_down_omp(&g, &sol3);
    end_time = CycleTimer::currentSeconds();
    printf("Top down OMP BFS time: %.3f sec\n", end_time - start_time);
	//*/

    // execute bottom-up version
    start_time = CycleTimer::currentSeconds();
    bfs_bottom_up_omp(&g, &sol1);
    end_time = CycleTimer::currentSeconds();
    ...
    for (int i=0; i<g.num_nodes; i++) {
        //*
		if (sol2.distances[i] != sol3.distances[i]) {
            fprintf(stderr, "***(sequential TD cmp with parallel TD) Distance results disagree at node %d: %d, %d\n", i, sol2.distances[i], sol3.distances[i]);
            exit(1);
        }
		//*/
		//*
		if (sol1.distances[i] != sol3.distances[i]) {
            fprintf(stderr, "***(sequential TD cmp with BU) Distance results disagree at node %d: %d, %d\n", i, sol2.distances[i], sol1.distances[i]);
            exit(1);
        }
		//*/
		//*
		if (sol2.distances[i] != sol4.distances[i]) {
            fprintf(stderr, "***(sequential TD cmp with Hybrid) Distance results disagree at node %d: %d, %d\n", i, sol2.distances[i], sol4.distances[i]);
            exit(1);
        }
		//*/
    }
